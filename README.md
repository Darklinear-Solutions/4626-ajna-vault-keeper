...
1. Configuration is split between `.env` (secrets and infrastructure) and `config.json` (operational parameters). Secrets stay in `.env` and should never be committed. All other configuration lives in `config.json`.
    ...
    **Config values (set in `config.json`):**
    ...
    | `oracle.onchainMaxStaleness`     | Max allowed age of on-chain price data. Set to `null` to disable the staleness check. | Integer (seconds) or `null` | Optional                                  | `null`           |
    | `oracle.fixedPrice`              | The keeper can be configured to skip both oracles and use a hard-coded price, defined here. Set to `null` to use the live oracle. | Float (human-readable price, e.g. `1.00`) or `null` | Optional | `null` |
...
4. Compute Targets:
    ...
    * The keeper reads the current pool price (`getPrice()`), converts it to a bucket index (`getPriceToIndex(price)`), then applies an integer offset `optimalBucketDiff` to produce `optimalBucket`, which `_getKeeperData()` stores for subsequent range checks.
